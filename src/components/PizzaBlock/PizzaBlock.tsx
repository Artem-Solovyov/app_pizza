import React, {useState} from 'react';
import {useDispatch, useSelector} from "react-redux";
import {addItems,} from "../../redux/cart/cartSlice";
import {selectCartItemById} from "../../redux/cart/cartSelectors";
import {CartItemType} from "../../redux/cart/cartTypes";
import {Link} from "react-router-dom";

const typesPizza = ['тонке', 'традиційне']
type PropsType = {
  id: string,
  title: string,
  price: number[],
  imageUrl: string,
  sizes: number[],
  types: number[]
}
const PizzaBlock: React.FC<PropsType> = ({id, title, price, imageUrl, sizes, types}) => {
    const cartItem = useSelector(selectCartItemById(id))
    const addCount = cartItem ? cartItem.count : 0
    const indexType = cartItem ? typesPizza.indexOf(cartItem.type) : 0
    const indexSize = cartItem ? sizes.indexOf(cartItem.size) : 0
    const [typePizza, setTypePizza] = useState(indexType ? indexType : 0);
    const [sizePizza, setSizePizza] = useState(indexSize ? indexSize : 0);


    const dispatch = useDispatch()

    const onClickAdd = () => {
      const item: CartItemType = {
        id,
        title,
        price: price[sizePizza],
        imageUrl,
        type: typesPizza[typePizza],
        size: sizes[sizePizza],
        count: 0
      }
      dispatch(addItems(item))
    }
    return (
      <div className="pizza-block">
        <Link to={'pizza/' + id}><img
          className="pizza-block__image"
          src={imageUrl}
          alt="Pizza"
        /></Link>
        <h4 className="pizza-block__title">{title}-піца</h4>
        <div className="pizza-block__selector">
          <ul>
            {types.map((type, index) => {
              return (
                <li key={type} onClick={() => setTypePizza(type)}
                    className={typePizza === index ? 'active' : ''}>{typesPizza[type]}</li>
              )
            })}
          </ul>
          <ul>
            {sizes.map((size, index) => {
              return (
                <li onClick={() => setSizePizza(index)} key={index}
                    className={sizePizza === index ? 'active' : ''}>{size} см.
                </li>
              )
            })}
          </ul>
        </div>
        <div className="pizza-block__bottom">
          <div className="pizza-block__price">від {price[sizePizza]}$</div>
          <button onClick={onClickAdd} className="button button--outline button--add">
            <svg
              width="12"
              height="12"
              viewBox="0 0 12 12"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M10.8 4.8H7.2V1.2C7.2 0.5373 6.6627 0 6 0C5.3373 0 4.8 0.5373 4.8 1.2V4.8H1.2C0.5373 4.8 0 5.3373 0 6C0 6.6627 0.5373 7.2 1.2 7.2H4.8V10.8C4.8 11.4627 5.3373 12 6 12C6.6627 12 7.2 11.4627 7.2 10.8V7.2H10.8C11.4627 7.2 12 6.6627 12 6C12 5.3373 11.4627 4.8 10.8 4.8Z"
                fill="white"
              />
            </svg>
            <span>Додати</span>
            {addCount > 0 && <i>{addCount}</i>}
          </button>
        </div>
      </div>
    );
  }
;

export default PizzaBlock;